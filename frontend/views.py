...
# Проверка, является ли пользователь сотрудником обслуживания номеров
def is_room_service(user):
    return user.groups.filter(name='Сотрудник обслуживания номеров').exists()

# Проверка, является ли пользователь либо сотрудником обслуживания номеров, либо сотрудником предоставления услуг отеля
...
# Проверка, что пользователь является либо сотрудником  для просмотра деталей заказа
@user_passes_test(is_room_or_hotel_service)
...
# Проверка, что пользователь является сотрудником обслуживания номеров для создания заказа
